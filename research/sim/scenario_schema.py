...
from app.core.models import LabelType, RiskTag, RiskType, SenderRole

# Default scenario directory (data/scenarios) relative to the repo root.
from app.core.paths import SCENARIOS_DIR as _SCENARIOS_DIR
...
class ScenarioInterruption(BaseModel):
    ...
    ``research.sim.tasks._resolve_trigger_minute`` accepts either an explicit
...
class ScenarioFile(BaseModel):
    ...
    Matches the keys read by ``research.sim.tasks.build_scenario`` via the loader:
