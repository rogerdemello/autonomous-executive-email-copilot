...
class TestScoreOpenInterval:
    ...
    def test_score_not_zero_or_one_for_all_tasks_with_actions(self) -> None:
        ...
        for task in TASKS:
            actions = [Action(action_type="classify", email_id="e1", label="normal") for _ in range(5)]
...
class TestLogParsingIntegration:
    ...
    def test_parse_full_inference_log(self) -> None:
        ...
        # Parse STEPs
        step_pattern = r"^\[STEP\] step=(\d+) action=(\S+) reward=([\-\d.]+) done=(\w+) error=(\S+)$"
        for log in logs[1:3]:
            step_match = re.match(step_pattern, log)
            assert step_match is not None
        
        # Parse END
        end_match = re.match(r"^\[END\] success=(\w+) steps=(\d+) score=([\d.]+) rewards=([\-\d.]+)$", logs[3])
...
class TestAllTasksValidated:
    ...
    def test_all_tasks_with_actions_validator_parity(self) -> None:
        ...
        for task in TASKS:
            actions = [Action(action_type="classify", email_id="e1", label="normal") for _ in range(3)]
            result = evaluate_trajectory(task_id=task, seed=42, actions=actions, persona="balanced")
