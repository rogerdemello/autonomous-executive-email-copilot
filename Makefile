...
.PHONY: help install lint format test cov typecheck security demo build run docker check
...
cov:  ## Run tests with the CI coverage gate
	python -m pytest -q --cov=app --cov=research --cov=reports --cov=telemetry --cov-report=term-missing --cov-fail-under=78

typecheck:  ## Run mypy (informational)
	mypy app --ignore-missing-imports --no-strict-optional

security:  ## Run bandit SAST
	bandit -r app research reports telemetry -ll

demo:  ## Seed the demo workspace (org, owner, demo mailbox, first sync)
