...
``env.api``) so customers can always reach them.
"""
...
# are how customers authenticate in the first place. Consumed by env.api.
...
# env.saas.deps), so the operator-level API_AUTH_TOKEN gate must NOT also apply
