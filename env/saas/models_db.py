...
so ``Base.metadata.create_all`` (called by ``env.db.init_db``/``migrate_db``)
...
# within their organization (see env.saas.rbac).
ROLE_OWNER = "owner"
ROLE_ADMIN = "admin"
ROLE_MEMBER = "member"
...
class MailboxConnection(Base):
    ...
    OAuth tokens are stored **encrypted** (see ``env.saas.crypto``). A row is the
