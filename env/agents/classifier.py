...
class ClassifierAgent(BaseAgent):
    ...
    def can_handle(self, observation: Observation) -> bool:
        for email in observation.emails:
            if email.priority_hint == "high" or self._contains_keywords(email.subject, URGENT_KEYWORDS):
                return True
    ...
    def execute(self, observation: Observation) -> Action | None:
        for email in observation.emails:
            label = self._determine_label(email.subject, email.body, email.sender_role, email.priority_hint)
            if label:
    ...
    def _determine_label(
        ...
        if self._contains_keywords(body, SPAM_KEYWORDS) or self._contains_keywords(subject, SPAM_KEYWORDS):
            return "spam"
